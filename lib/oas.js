 ...
 * @returns {string}
 */
function toLabel(service, useFilenameAsTagPrefix) {
  return useFilenameAsTagPrefix ?
    `${path.basename(service.filename, '.proto')}.${service.name}` : service.fullName.substring(1);
}
...
function toPaths(root, service, options) {
  const {
    routes,
    useFilenameAsTagPrefix,
  } = options;
  const namespace = service.parent.name;
  const tag = toLabel(service, useFilenameAsTagPrefix);
        ...
        if (params.length < requestType.fieldsArray.length) {
          pathSpec[action].parameters = [
            ...(pathSpec[action].parameters || []),
            {
              ...
              required: false,
              schema: {
                $ref: `#/components/schemas/${namespace}.${requestType.name}`,
              },
            },
          ];
        }
      ...
      pathSpec[action].responses = {
        200: {
          ...
          content: {
            'application/json': {
              schema: {
                $ref: `#/components/schemas/${namespace}.${responseType.name}`,
              },
            },
          }
        },
 ...
 * @returns {Object}
 */
function toTag(service, options) {
  const {
    useFilenameAsTagPrefix = false,
  } = options;
  const name = toLabel(service, useFilenameAsTagPrefix);
