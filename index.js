 ...
 * @param {boolean} [options.useFilenameAsTagPrefix=false] - Use "filename.Service" format as tag.
 ...
 * 
 * @example
 * pb2oas('user.proto', { routes: { 'User.getUserInfo': 'get /users/:userId' } });
 * 
 * @returns {Object} OpenAPI JSON Object.
 */
 function pb2oas(path, options = {}) {
  if (!path) {
    throw new Error('pb2oas Error: "path" is required.');
  }

  if (!options.title) {
    throw new Error('pb2oas Error: "options.title" is required.');
  }

  if (!Array.isArray(options.servers) || !options.servers.length) {
    throw new Error('pb2oas Error: "options.servers" must be a non-empty array');
  }

  options.servers.forEach((server) => {
    if (!server.url) {
      throw new Error('pb2oas Error: "servers[].url" is required.');
    }
  });

  if (!fs.existsSync(path)) {
    throw new Error(`pb2oas Error: "${path}" does not exists.`);
  }
  ...
  if (!filenames.length) {
    throw new Error(`pb2oas Error: .proto files was not found`)
  }

  options.routes = options.routes || {};
  options.keepCase = options.keepCase || true;
  options.useFilenameAsTagPrefix = options.useFilenameAsTagPrefix || false;

  return parse(filenames, options);
}

module.exports = pb2oas;
